...
def get_vision_transformer():
    ...
    patches = ImagePatches(33)(image_input) #Extract the patches and flatten them
    linear_projection = Dense(64)(patches) #Linear Projection Layer
    position_embedding = Embedding(9,64)(tf.range(0, 9,1)) #Positional embedding
    ...
    for i in range(3):
        ...
        mlp_output = Dense(64)(tokens_1_norm) #Fully connected layer
    ...
    tokens_final = Flatten()(tokens_final) #Flatten the final outputs
    tokens_final = Dropout(0.5)(tokens_final) #Dropout layer to avoid overfitting

    output = Dense(128, activation='relu', kernel_regularizer=l2(0.01))(tokens_final) #Fully connected layer
...
def get_text_encoder():
    ...
    model.add(Embedding(max_features, 32, input_length=31))
...
def match_model():
    ...
    image_embedding = get_vision_transformer()(image_input) #call the image encoder
...
optimizer = tf.keras.optimizers.Adam(learning_rate=0.00005, clipnorm=1.0) #get the optimizer
